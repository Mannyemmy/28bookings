...
const MoreItems = () => {
    ...
    const { data : items, error, isLoading, isFetching, isSuccess } =
  ...
  return (
    <div className="more-items px-1 px-md-3 mt-4 mt-md-2">
           ...
           <div className="d-flex justify-content-between p-0 m-0 mt-3">
               <div className='prev-btn-wrapper tw-mr-1 h-100 d-flex align-items-center m-hide'>
                    <button onClick={ handlePrevClick } className='btn btn-success prev-btn bi bi-chevron-left'></button>
               </div>
               <div className="product-wrapper more-items-wrapper h-100 d-flex flex-nowrap tw-space-x-2 scrolling-container">
                     {
                        items && items.map( (item) => {
                             return (
                                 <div onClick={ () => handleNavigateToRentalPage( item.slug ) }
                                  className=" h-100 tw-cursor-pointer" key={item.id}>
               ...
               <div className=" tw-ml-1 next-btn-wrapper h-100 d-flex align-items-center m-hide">
